...
function routerFormat(router, staticDefinenations, sequelize) {
    ...
    if (sequelize) {
        try {
            ...
            console.warn('swagger-express-option-sequelize 未找到模型 - ' + sequelize.modelPath + router.name)
        }
    ...
    if (router && router.router && router.router.stack) {
        for (const api of router.router.stack) {
            ...
            for (const currentMethod of currentMethods) {
                ...
                const uri = `/${router.name.toLowerCase()}${api.route.path.toLowerCase()}`
                ...
                const currentDocRule = routesDocRules.defaultAPIs[api.route.path.toLowerCase()]
                ...
                if (currentDocRule && currentModel) {
                    ...
                    parameters[uri][currentMethod]['parameters'] = [staticDefinedObjects[currentDocRule.request]]
