...
* @Last Modified time: 2017-07-04 15:54:18
*/
...
describe('PUT /user', function() {
  it('Simple Put Test', function(done) {
    server
      .post('/user')
	 	  .send({name: 'toto', dob: dateFormat(now, 'isoDateTime'),
     address: 'Shanghai', description: 'This user is a test'})
      .expect('Content-type', /json/)
	    .expect(200)
	    .end(function(err, res) {
	      res.status.should.equal(200);
  ...
  it('Test mandatory field : name', function(done) {
    server
      .post('/user')
      .send({dob: dateFormat(now, 'isoDateTime'),
        address: 'Shanghai', description: 'This user is a test'})
      .expect('Content-type', /json/)
	    .expect(422)
	    .end(function(err, res) {
  ...
  it('Test mandatory field : dob', function(done) {
    server
      .post('/user')
      .send({name: 'titi', address: 'Shanghai',
        description: 'This user is a test'})
      .expect('Content-type', /json/)
	    .expect(422)
	    .end(function(err, res) {
	     res.status.should.equal(422);
    ...
    server
      .post('/user')
      .send({name: 'tutu', dob: dateFormat(now, 'isoDateTime'),
        description: 'This user is a test'})
      .expect('Content-type', /json/)
	    .expect(422)
	    .end(function(err, res) {
    ...
    server
      .post('/user')
      .send({name: 'tata', dob: dateFormat(now, 'isoDateTime'),
        address: 'Paris'})
      .expect('Content-type', /json/)
	    .expect(200)
	    .end(function(err, res) {
  ...
  describe('PUT /user - Name field rules', function() {
    it('Test Name Min lenght', function(done) {
      server
        .post('/user')
		 	  .send({name: 'ab', dob: dateFormat(now, 'isoDateTime'),
     address: 'Shanghai', description: 'This user is a test'})
        .expect('Content-type', /json/)
		    .expect(422)
		    .end(function(err, res) {
    ...
    it('Test Name Max lenght', function(done) {
      server
        .post('/user')
		 	  .send(
        {name: 'Hello my name is way to long to be a correct name\
          , so this will fail',
          dob: dateFormat(now, 'isoDateTime'), address: 'Shanghai',
          description: 'This user is a test'})
        .expect('Content-type', /json/)
		    .expect(422)
		    .end(function(err, res) {
...
describe('PATCH /user/{id} ', function() {
  it('Test Update existing user', function(done) {
    server
      .patch('/user/' + savedId)
	 	  .send({name: 'tyty', address: 'Tokyo',
     description: 'This user is a test and has been updated'})
      .expect('Content-type', /json/)
	    .expect(200)
	    .end(function(err, res) {
	     res.status.should.equal(200);
          ...
          'This user is a test and has been updated');
