...
const generateFriendship = properties => ({
  ...friendshipSample,
  ...properties,
});
...
describe("Friendship model", () => {
  ...
  it("should list friendships", async () => {
    ...
    expect((await Friendship.list({ limit: 10000 })).docs.length).toEqual(
      friendships.length
    );
    expect((await Friendship.list({ limit: 10 })).docs.length).toEqual(10);
    expect((await Friendship.list({ limit: 10000 })).docs[0].toJSON()).toEqual(
      friendships[22]
    );
    expect(
