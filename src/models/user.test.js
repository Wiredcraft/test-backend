...
describe("User model", () => {
    ...
    it("should list users", async () => {
        ...
        expect((await User.list({ limit: 10000 })).docs.length).toEqual(users.length);
        expect((await User.list({ limit: 10 })).docs.length).toEqual(10);
        expect((await User.list({ limit: 10000 })).docs[0].toJSON()).toEqual(users[22])
        expect((await User.list({ offset: 10, limit: 10 })).docs[0].toJSON()).toEqual(users[12])
    });
});
